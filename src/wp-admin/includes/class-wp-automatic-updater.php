...
=========='
	...
	protected function has_fatal_error() {
		...
		if ( false === $scrape_result_position ) {
			// Error detection is not possible when scraping_result is not found, e.g. when loopback request to homepage is redirected without scraping parameters 
			if ( $is_debug ) {
				error_log( 'Error detection is not possible when scraping_result is not found, e.g. when loopback request to homepage is redirected without scraping parameters' );
			}
			return false;
		}else{
			...
			if ( $is_debug ) {
				// error_output is result of wp_json_encode(), either with ( true ) or with ( error_get_last() ) when error is detected while scraping
